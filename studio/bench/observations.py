...
class Collector:
    ...
    def reserve(self, operation):
        with self.lock:
            ...
            self.started+=1
            record = {'call_id':uuid.uuid4().hex,'test_id':self.test_id,'operation':operation,'created_at':time.time(),
                      'status':0,'ok':False,'usage':{'input':None,'cached':None,'output':None},
                      'usage_present':False,'elapsed_ms':None,'adapter_version':'openai-usage-v1'}
