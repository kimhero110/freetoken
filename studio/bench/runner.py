...
def run_benchmark(base_url, api_key, model, test_ids, progress=None, collector=None, probe_config=None):
    ...
    client = Client(base_url, api_key, model, collector=collector)
    ...
    for tid in ORDER:
        if tid not in valid:
            continue
        if client.collector.cancelled.is_set():
            break
        client.collector.test_id = tid
        meta = TESTS[tid]
        if progress:
    ...
    run = {
        ...
        "observations": client.collector.snapshot(),
        "composite": composite,
        "tests": results,
    }
    return run
