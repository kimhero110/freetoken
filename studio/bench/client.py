...
from .observations import Collector
...
class Client:
    def __init__(self, base_url, api_key, model="", collector=None):
    ...
    def snapshot_usage(self):
        calls = [r for r in self.collector.snapshot() if r['operation'] == 'chat']
        def total(key):
            values = [r['usage'][key] for r in calls]
            return None if any(v is None for v in values) else sum(int(v) for v in values)
        return {'requests': len(calls), 'prompt_tokens': total('input'),
                'completion_tokens': total('output'),
                'observed_usage_calls': sum(r['usage_present'] for r in calls)}
    ...
    def chat(self, **kwargs):
        record = self.collector.reserve('chat')
        try:
            result = self._chat(**kwargs)
        except Exception:
            result = {'ok': False, 'status': 0, 'error': '调用异常', 'usage': None,
                      'content': '', 'elapsed_ms': 0, 'ttft_ms': None, 'tool_calls': [],
                      'finish_reason': None, 'chunk_times': [], 'resp_headers': {}}
        self.collector.finish(record, result)
        return result

    def _chat(self, model=None, messages=None, stream=False, max_tokens=512,
        ...
        try:
            ...
            r["error"] = str(e)[:300]
        if r["usage"]:
            pass  # authoritative metering is recorded by Collector
        return r
